...
const PLACEHOLDER_COLORS: Record<string, string> = {
  world_map: '#44cc44', character: '#ffd700', bag: '#ff8844', spirit: '#b44aff',
  skill: '#4488ff', bone: '#ffcc44', arena: '#ff6644', sect: '#cc88ff',
  quest: '#ffd700', rank: '#44aaff', settings: '#9a8e7a',
  codex: '#44cc88', smithy: '#ff8844', shop: '#ffd700', academy: '#44aaff',
  lock: '#666666', map_pin: '#ff4a4a', boss: '#ff4444', city: '#ffd700',
}
...
const SYSTEM_CHARS: Record<string, string> = {
  world_map: '🌍', character: '👤', bag: '🎒', spirit: '🔮',
  skill: '⚡', bone: '🦴', arena: '⚔️', sect: '🏰',
  quest: '📋', rank: '🏆', settings: '⚙️', codex: '📖',
  smithy: '🔨', shop: '🏪', academy: '📚', city: '🏙️',
  lock: '🔒', map_pin: '📍', boss: '👹',
}
...
export function boneIconPath(id: string): string {
  ...
  return iconUrls[path] ?? placeholderSvg('bone', '🦴')
}
...
export function itemIconPath(id: string): string {
  ...
  return iconUrls[path] ?? placeholderSvg(id, id.slice(0, 1).toUpperCase())
}
